...
namespace PipelineDemo
{
    public class ApplicationBuilder
    {
        ...
        //{
        //    middleware(new HttpContext(), ()=> { return null; });
        //    return this;
        //}
        ...
        public ApplicationBuilder Use(Func<HttpContext, Func<Task>, Task> middleware)
        {
            ...
            // return Use(next => { return context => { return null; }; }) ;

            //_components里全是这些方法
            return Use(next =>
            {
                //next(new HttpContext());
                //return null;

                return context =>
                {
                    //声明本地函数SimpleNext,这里只是声明，调用的时候才会执行next(context)
                    // Task SimpleNext() =>next(context);

                    Task SimpleNext() { return next(context); }



                    // Func<Task> f = SimpleNext;
